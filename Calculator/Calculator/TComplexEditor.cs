...
{
    public sealed class TComplexEditor : TAEditor 
    {
        private string number;
        const string Separator = " + i * ";
        readonly Regex ZeroComplex = new Regex(@"^-?(0+.?0*)(\s*\+\s*i\s*\*\s*-?(0+.?0*)|(\s*\+\s*i\s*\*\s*-?))?$");
        ...
        public override string AddZero() 
        {
            if (number == "0" || number == "-0" 
                || number.EndsWith(" 0") || number.EndsWith(" -0") 
                || number.EndsWith(Separator))
			{
                return number;
            }
            number += "0";
